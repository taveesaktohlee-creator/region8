...
import { ArrowLeft, ArrowRight, CheckCircle2, FileText, Loader2, MessageSquare, Timer, Trash2 } from 'lucide-react';
          ...
          <section ref={viewerRef} className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  <MessageSquare size={16} /> {annotationMode === 'point' ? 'คลิกเพื่อคอมเมนต์' : 'คอมเมนต์'}
                </button>
                {pdfOpenUrl && (
    ...
    <div className="grid gap-4 bg-slate-100 p-3 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="relative h-[78vh] min-h-[620px] w-full overflow-hidden bg-slate-100">
        ...
        {draftComment && (
          <DraftCommentBubble
            style={{
              left: `${Math.min(draftComment.xPercent, 70)}%`,
      ...
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1120px)_320px]">
        ...
        >
          ...
          {draftComment && (
            <DraftCommentBubble
              style={{
                left: `${Math.min(draftComment.xPercent, 74)}%`,
                top: `${draftComment.yPercent}%`,
              }}
              onSave={onSaveNewComment}
              onCancel={onCancelDraft}
...
function EditableCommentBubble({
  ...
  return (
    <div
      className={`relative w-full min-w-0 rounded-2xl border border-amber-200 bg-amber-50/95 p-3 text-left shadow-lg backdrop-blur ${canEdit ? 'cursor-text' : 'cursor-default'}`}
      onClick={(event) => {
