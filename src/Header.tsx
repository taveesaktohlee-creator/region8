...
const Header: React.FC<HeaderProps> = ({ setIsSidebarOpen, handleRefresh, isRefreshing, handleLogout }) => {
  ...
  return (
    <header className="flex items-center justify-between px-3 py-4 sm:px-8 sm:py-5 sticky top-0 z-30 bg-[#F6F6F6]/90 backdrop-blur-3xl border-b border-black/5 transition-all">
      <div className="flex min-w-0 items-center gap-2 sm:gap-4">
        <button type="button" onClick={() => setIsSidebarOpen(prev => !prev)} className="shrink-0 p-2 text-slate-500 hover:text-slate-900 hover:bg-white/60 rounded-xl transition-all shadow-sm lg:hidden border border-white/50">
          <PanelLeft size={22} />
        </button>
        ...
        <h1 className="shrink-0 text-xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-slate-800 to-slate-500 tracking-tight drop-shadow-sm sm:text-2xl">
          สตท.8
        </h1>
      </div>
      ...
      <div className="ml-2 flex shrink-0 items-center gap-2 sm:gap-3">
        <button
          ...
          className="p-2 bg-white/70 backdrop-blur-md text-slate-600 rounded-full shadow-sm border border-white/60 hover:bg-white hover:shadow-md transition-all group cursor-pointer sm:p-2.5"
          aria-label="หน้าหลัก"
          title="หน้าหลัก"
        >
          <Home size={18} className="group-hover:-translate-y-0.5 transition-transform" />
        </button>
        <button type="button" onClick={handleRefresh} className="p-2 bg-white/70 backdrop-blur-md text-slate-600 rounded-full shadow-sm border border-white/60 hover:bg-white hover:shadow-md transition-all group cursor-pointer sm:p-2.5">
        ...
        <div ref={notificationPanelRef} className="relative">
          <button
            ...
            className="p-2 bg-white/70 backdrop-blur-md text-slate-600 rounded-full shadow-sm border border-white/60 hover:bg-white hover:shadow-md transition-all relative cursor-pointer sm:p-2.5"
        ...
        <button type="button" onClick={handleLogout} className="p-2 bg-white/70 backdrop-blur-md text-slate-600 rounded-full shadow-sm border border-white/60 hover:bg-white hover:shadow-md transition-all group cursor-pointer sm:p-2.5">
